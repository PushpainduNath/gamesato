 ...
 */
router.post('/', auth_1.authenticate, auth_1.requireAdmin, fileManager_1.upload.single('icon'), async (req, res) => {
    const { name } = req.body;
    ...
    try {
        const existing = await db_1.pool.query('SELECT id FROM categories WHERE LOWER(name) = LOWER($1) OR slug = $2', [name, slug]);
        if (existing.rows.length > 0) {
            if (req.file) {
                await fs_1.promises.unlink(req.file.path).catch(console.error);
            }
            return res.status(400).json({ error: 'Category name or slug already exists' });
        }
        let finalIconUrl = icon || '/arcade.svg';
        if (req.file) {
            finalIconUrl = await (0, fileManager_1.saveCategoryIcon)(req.file.path, slug, req.file.originalname);
        }
        const result = await db_1.pool.query('INSERT INTO categories (name, slug, icon) VALUES ($1, $2, $3) RETURNING *', [name, slug, finalIconUrl]);
 ...
 */
router.put('/:id', auth_1.authenticate, auth_1.requireAdmin, fileManager_1.upload.single('icon'), async (req, res) => {
    const { id } = req.params;
    const { name } = req.body;
    let { slug, icon } = req.body;
    if (!name) {
    ...
    try {
        const currentRes = await db_1.pool.query('SELECT name, icon, content, status, meta_title, meta_description, meta_tags FROM categories WHERE id = $1', [id]);
        ...
        const conflictRes = await db_1.pool.query('SELECT id FROM categories WHERE (LOWER(name) = LOWER($1) OR slug = $2) AND id <> $3', [name, slug, id]);
        ...
        if (req.file) {
            ...
            if (oldIcon && oldIcon.startsWith('/uploads/icons/')) {
                await (0, fileManager_1.deleteLocalImage)(oldIcon).catch(console.error);
            }
        }
        ...
        try {
            ...
            const updateRes = await client.query(`UPDATE categories 
         SET name = $1, slug = $2, icon = $3, content = $4, status = $5, 
             meta_title = $6, meta_description = $7, meta_tags = $8, updated_at = CURRENT_TIMESTAMP 
         WHERE id = $9 RETURNING *`, [name, slug, finalIconUrl, newContent, newStatus, newMetaTitle, newMetaDescription, newMetaTags, id]);
            await client.query('UPDATE games SET category = $1 WHERE LOWER(category) = LOWER($2)', [name, oldName]);
            ...
            if (req.file) {
                const uploadedPath = path_1.default.join(__dirname, '../..', finalIconUrl);
                await fs_1.promises.unlink(uploadedPath).catch(console.error);
            }
            throw txErr;
        }
        finally {
...
router.delete('/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    try {
        const currentRes = await db_1.pool.query('SELECT name, icon FROM categories WHERE id = $1', [id]);
        if (currentRes.rows.length === 0) {
            return res.status(404).json({ error: 'Category not found' });
        }
        ...
        const gamesCountRes = await db_1.pool.query('SELECT COUNT(*)::int as count FROM games WHERE LOWER(category) = LOWER($1)', [catName]);
        ...
        if (gamesCount > 0) {
            ...
            let targetRes;
            if (targetCategoryId) {
                targetRes = await db_1.pool.query('SELECT name FROM categories WHERE id = $1 AND id <> $2', [targetCategoryId, id]);
            }
            else {
                targetRes = await db_1.pool.query('SELECT name FROM categories WHERE LOWER(name) = LOWER($1) AND id <> $2', [targetCategoryName, id]);
            }
        ...
        try {
            ...
            if (gamesCount > 0 && destinationCategoryName) {
                await client.query('UPDATE games SET category = $1 WHERE LOWER(category) = LOWER($2)', [destinationCategoryName, catName]);
            }
            await client.query('DELETE FROM categories WHERE id = $1', [id]);
            await client.query('COMMIT');
            if (catIcon && catIcon.startsWith('/uploads/icons/')) {
                await (0, fileManager_1.deleteLocalImage)(catIcon).catch(console.error);
            }
            await invalidateCache();
            res.json({
 ...
 */
router.post('/:id/games', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        const catRes = await db_1.pool.query('SELECT name FROM categories WHERE id = $1', [id]);
        if (catRes.rows.length === 0) {
            return res.status(404).json({ error: 'Category not found' });
        }
        const catName = catRes.rows[0].name;
        await db_1.pool.query('UPDATE games SET category = $1 WHERE id = ANY($2::uuid[])', [catName, gameIds]);
        await invalidateCache();
 ...
 */
router.delete('/:id/games', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        await db_1.pool.query('UPDATE games SET category = $1 WHERE id = ANY($2::uuid[]) AND LOWER(category) = LOWER($3)', [destinationCategoryName, gameIds, catName]);
        await invalidateCache();
