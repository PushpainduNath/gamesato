...
router.post('/login', async (req, res) => {
    ...
    try {
        const result = await db_1.pool.query('SELECT id, name, email, role, password_hash, is_blocked FROM admin_users WHERE email = $1', [username]);
        ...
        const isMatch = bcryptjs_1.default.compareSync(password, admin.password_hash);
        if (!isMatch) {
            return res.status(401).json({ error: 'Invalid email or password' });
        }
        // Generate JWT token
        const token = jsonwebtoken_1.default.sign({ id: admin.id, name: admin.name, email: admin.email, role: admin.role }, JWT_SECRET, { expiresIn: '24h' });
        res.json({
            token,
            admin: {
                id: admin.id,
...
router.get('/dashboard', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    ...
    `);
        ...
        const totalUsersRes = await db_1.pool.query(totalUsersQuery, params);
        // Total Games
        const totalGamesQuery = hasFilter
            ? "SELECT COUNT(*) FROM games WHERE created_at >= $1::timestamp AND created_at <= $2::timestamp"
            : "SELECT COUNT(*) FROM games";
        const totalGamesRes = await db_1.pool.query(totalGamesQuery, params);
        ...
        const totalPlaysRes = await db_1.pool.query(totalPlaysQuery, params);
        ...
        const avgDurationRes = await db_1.pool.query(avgDurationQuery, params);
       ...
       SELECT g.id, g.title, g.slug, g.category, g.thumbnail_url, g.status, g.description, g.how_to_play, g.is_featured,
    ...
    `;
        const gamesStatsRes = await db_1.pool.query(gamesStatsQuery, params);
        // Distribution by category
        const categoryDistributionQuery = `
      SELECT g.category, 
             COUNT(DISTINCT g.id) as count, 
             COALESCE(SUM(GREATEST(COALESCE(g.play_count, 0), COALESCE(ae.plays, 0))), 0)::int as total_plays
      FROM games g
      LEFT JOIN (
      ...
      GROUP BY g.category
    `;
        const categoryDistributionRes = await db_1.pool.query(categoryDistributionQuery, params);
        ...
        const dailyPlaysQuery = hasFilter
         ...
         GROUP BY DATE_TRUNC('day', created_at)
         ORDER BY date ASC`;
        const dailyPlaysRes = await db_1.pool.query(dailyPlaysQuery, params);
        const payload = {
            summary: {
...
router.get('/users', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        const status = req.query.status || 'all'; // 'all', 'active', 'blocked'
        const provider = req.query.provider || 'all'; // 'all', 'credentials', 'google', 'facebook', 'discord'
        const sortBy = req.query.sortBy || 'newest'; // 'newest', 'oldest', 'name_asc', 'name_desc'
        ...
        const countResult = await db_1.pool.query(countQuery, queryParams);
    ...
    `;
        ...
        const result = await db_1.pool.query(selectQuery, queryParams);
        res.json({
            users: result.rows,
            pagination: {
                totalCount,
 ...
 */
router.put('/users/:id/block', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        const result = await db_1.pool.query("UPDATE users SET is_blocked = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING id, name, email, 'USER'::varchar as role, is_blocked", [is_blocked, id]);
 ...
 */
router.put('/users/:id/role', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
 ...
 */
router.get('/admins', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
    try {
        const result = await db_1.pool.query('SELECT id, name, email, role, plain_password, is_blocked, created_at FROM admin_users ORDER BY role DESC, created_at DESC');
 ...
 */
router.post('/admins', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
    ...
    try {
        const checkUser = await db_1.pool.query('SELECT id FROM admin_users WHERE email = $1', [email]);
        if (checkUser.rows.length > 0) {
            return res.status(400).json({ error: 'Email address already registered' });
        }
        const passwordHash = bcryptjs_1.default.hashSync(password, 10);
        const result = await db_1.pool.query(`INSERT INTO admin_users (email, name, role, password_hash, plain_password)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, name, email, role, plain_password, is_blocked, created_at`, [email, name || null, role, passwordHash, password]);
 ...
 */
router.put('/admins/:id/role', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
    const { id } = req.params;
    const { role } = req.body;
    if (role !== 'ADMIN' && role !== 'SUPER_ADMIN') {
    ...
    try {
        const result = await db_1.pool.query('UPDATE admin_users SET role = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING id, name, email, role, is_blocked', [role, id]);
        if (result.rowCount === 0) {
 ...
 */
router.put('/admins/:id/block', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
    const { id } = req.params;
    const { is_blocked } = req.body;
    if (typeof is_blocked !== 'boolean') {
        return res.status(400).json({ error: 'is_blocked field must be a boolean' });
    ...
    try {
        const result = await db_1.pool.query('UPDATE admin_users SET is_blocked = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING id, name, email, role, is_blocked', [is_blocked, id]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: 'Admin user not found' });
        }
        res.json(result.rows[0]);
 ...
 */
router.delete('/admins/:id', auth_1.authenticate, auth_1.requireSuperAdmin, async (req, res) => {
    return res.status(403).json({ error: 'User deletion is disabled. Please block the user instead.' });
});
/* ========================================================
 ...
 */
router.post('/cleanup', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const activeGamesRes = await db_1.pool.query('SELECT slug FROM games');
        const activeSlugs = activeGamesRes.rows.map(g => g.slug);
        await (0, fileManager_1.cleanOrphanedDirectories)(activeSlugs);
 ...
 */
router.post('/cache/flush', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
 ...
 */
router.get('/content/pages/public/:slug', async (req, res) => {
    const { slug } = req.params;
    try {
        const result = await db_1.pool.query('SELECT * FROM static_pages WHERE LOWER(slug) = LOWER($1)', [slug]);
        if (result.rows.length === 0) {
 ...
 */
router.get('/content/pages', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const result = await db_1.pool.query('SELECT * FROM static_pages ORDER BY title ASC');
        res.json(result.rows);
    }
    catch (err) {
 ...
 */
router.put('/content/pages/:id', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        const result = await db_1.pool.query(`UPDATE static_pages 
       SET title = $1, content = $2, status = $3, meta_title = $4, meta_description = $5, meta_tags = $6, updated_at = CURRENT_TIMESTAMP 
       WHERE id = $7 RETURNING *`, [title, content || '', status || 'published', meta_title || null, meta_description || null, meta_tags || null, id]);
        if (result.rowCount === 0) {
...
router.get('/settings', async (req, res) => {
    ...
    `);
        const result = await db_1.pool.query('SELECT key, value FROM site_settings');
        const settingsMap = {
