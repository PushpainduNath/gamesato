...
async function run() {
  try {
    ...
    // 3. Seed 1100 Dummy Users (so we can generate > 1000 likes per game)
    ...
    // Game 1: above 100 (e.g. 150 likes)
    // Game 2: in 700s (e.g. 750 likes)
    // Game 3: in 900s (e.g. 950 likes)
    ...
    if (gamesForLikes.length >= 3) {
      ...
      const headSoccer = gamesForLikes.find(g => g.slug === 'head-soccer-2026') || gamesForLikes[1];
      const subwayRiders = gamesForLikes.find(g => g.slug === 'subway-riders') || gamesForLikes[2];

      const likesToInsert = [];

      // 1050 likes for Cut The Candy (above 1000)
      ...
      // 750 likes for Head Soccer 2026 (in 700s)
      for (let i = 0; i < 750; i++) {
        const u = usersForLikes[i];
        likesToInsert.push({
          userId: u.id,
      ...
      // 950 likes for Subway-Riders (in 900s)
      for (let i = 0; i < 950; i++) {
        const u = usersForLikes[i];
        likesToInsert.push({
          userId: u.id,
    ...
    // 5. Seed Analytics Events (plays and HEARTBEATS to calculate average durations)
    console.log('Generating gameplay analytics sessions for the last 30 days...');
    
    // Fetch games and users to map events to them
    ...
    const headSoccerId = gamesRows.find(g => g.slug === 'head-soccer-2026')?.id || gamesRows[1]?.id;
    ...
    for (let s = 0; s < sessionCount; s++) {
      // Weighted distribution to create differentiated play counts:
      // Cut Candy: ~60% of plays
      // Head Soccer: ~30% of plays
      // Subway Riders: ~10% of plays
      let gameId;
      ...
      // 20% of plays are by anonymous users (null userId)
      ...
      // Determine session start time in the last 30 days (must be after user creation time if registered)
    ...
    // Bulk insert events in batches of 1000 rows
    const batchSize = 1000;
